...
def shape_element(element, node_attr_fields=NODE_FIELDS, way_attr_fields=WAY_FIELDS,
    ...
    if element.tag == 'node':
            ...
            for tag in element.iter('tag'):
                ...
                if LOWER_COLON.search(tag.attrib['k']):
                    ...
                    tags.append(nodes)
                elif LOWER_COLON.search(tag.attrib['k']) == None:
                    nodes['type'] = 'regular'
    ...
    if element.tag == 'way':
            ...
            for tag in element.iter('tag'):
                tagt = {}
                tagt['id'] = element.attrib['id']
                tagt['value'] = tag.attrib['v']
                if tag.attrib['k'] == 'addr:street':
                    #update street names
                    tagt['value'] = update(tag.attrib['v'])
                 #identifying the type in the CSV file    
                if LOWER_COLON.search(tag.attrib['k']):
                    tagt['type'] = tag.attrib['k'].split(':',1)[0]
                    tagt['key'] = tag.attrib['k'].split(':',1)[1]
                    tags.append(tagt) 
                elif LOWER_COLON.search(tag.attrib['k']) == None:
                    tagt['type'] = 'regular'
                    tagt['key'] = tag.attrib['k']
                    tags.append(tagt)    
...
def process_map(file_in, validate):
    ...
    with codecs.open(NODES_PATH, 'wb') as nodes_file,          codecs.open(NODE_TAGS_PATH, 'wb') as nodes_tags_file,          codecs.open(WAYS_PATH, 'wb') as ways_file,          codecs.open(WAY_NODES_PATH, 'wb') as way_nodes_file,          codecs.open(WAY_TAGS_PATH, 'wb') as way_tags_file:
        ...
        #validator = cerberus.Validator() cerberus not available Python 2.7
